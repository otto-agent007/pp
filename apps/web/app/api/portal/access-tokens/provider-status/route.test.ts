...
function request() {
  return new Request("http://localhost/api/portal/access-tokens/provider-status");
}

describe("portal provider status route", () => {
  beforeEach(() => {
    adminError = null;
    vi.unstubAllEnvs();
  });
  ...
  it("reports manual fallback without exposing provider secrets", async () => {
    vi.stubEnv("PORTAL_DELIVERY_WEBHOOK_SECRET", "secret-value");
