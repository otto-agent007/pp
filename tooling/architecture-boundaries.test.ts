...
// Annotated so the returned literal keeps its union types instead of widening
// to `string`. The overrides stay a Partial of the same type: the tests that
// build deliberately invalid exceptions do so with type-valid values — a
// non-normalized path is still a string — and it is the validator's job, not
// the compiler's, to reject them.
...
describe("architecture policy", () => {
  ...
  it("rejects invalid exception occurrence values and paths outside the importer", () => {
    const policy = validPolicy();
    // `syntax` and `occurrenceClass` are deliberately outside their unions.
    // The policy is loaded from JSON, so values like these are reachable at
    // runtime and rejecting them is exactly what is under test; the cast lets
    // the fixture stay typed everywhere else.
    policy.exceptions = [exceptionWith({ sourceOccurrences: [{
      ...
      count: 0,
      bindingDigest: "ABC",
    }] as unknown as ArchitectureException["sourceOccurrences"] })];
  ...
  it("rejects invalid manifest pairs, missing-manifest evidence, and removal metadata", () => {
    ...
    policy.exceptions = [
      ...
      exceptionWith({
        id: "b",
        kind: "forbidden-workspace-edge",
        // `section` is deliberately not a ManifestSection; see above.
        manifest: {
          section: "invalid",
          versionSpecifier: "",
        } as unknown as ArchitectureException["manifest"],
      }),
    ];
