...
Current priority is launch-readiness evidence and white-canvas UI polish while protected-preview smoke remains operator-gated:
...
- `@pest-patrol/ui-native` now provides shared token-driven React Native primitives for mobile buttons, cards, eyebrows, status pills, sync badges, stat tiles, and avatars
...
- demo seed/reset now carries a larger San Diego proof story with 12 technicians, 18 customers, 26 locations, 30 jobs, uploaded synthetic service-photo/signature media, media-aware summary counts, checked-in local fixture assets for proof rendering without Supabase storage access, and automatic `demo@email.com` login refresh guarded away from production
...
- current status: California Compliance RAG V1 plus source-ingestion tooling are implemented as an advisory-only baseline, the dry-run/no-embed source preflight passes locally, the compliance advisory route checks schema/RPC readiness before OpenAI embeddings, `/compliance` now makes non-live/advisory runtime/audit states clearer, the customer/admin shared-primitives closeout is locally verified, status/searchability QA is locally verified with fixture Browser route walking, local fixture demo data is visible without Supabase auth or seed-route access, tokened local `/portal` renders the Rivera Cafe proof media from fixtures, the mobile native primitives package is verified and lightly adopted in the technician header/job card plus sync status badge, the brand-font direction bundle is design-only guidance, the `/auth/update-password` Vercel lambda mapping blocker is fixed, local Windows Vercel packaging is now blocked by a symlink `EPERM`, and pending migration application remains gated on an approved local or preview target; no preview/production migration has been applied by Codex; the latest Ready preview is `https://pest-patrol-2ayfmsfpw-ottoagent007-gmailcoms-projects.vercel.app`; the latest Ready production deployment is `https://pest-patrol-m084wbv4s-ottoagent007-gmailcoms-projects.vercel.app`; local fixture browser smoke passed on `/`, `/dispatch`, `/customers`, `/jobs`, `/inventory`, `/payments`, `/closeouts`, `/compliance`, `/automation`, and tokened `/portal` at desktop and narrow widths with a white sampled canvas, no console/page errors, and no horizontal overflow; real local and preview seed/reset preflights remain blocked on approved Supabase env names, local Docker/Postgres availability for local Supabase target inspection, protected-preview access, and admin/dispatcher sign-in
