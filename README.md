...
- `/payments` remains the invoice workspace with closeouts, reconciliation, customer, and portal handoffs, including fresh customer-portal QR/link generation for invoice sharing
- `/customers` now includes compact account follow-up status, expandable account ledger drill-downs, portal-token readiness, portal handoff review, provider status, fresh-token send, send-attempt history, and session-only QR/share cards for freshly generated portal links
- tokened `/portal` routes now include a customer-safe account overview, open invoice payment actions, service and billing history, proof-of-service cards, and a General Pest recurring-service request CTA
...
- `/technicians` now shows provider-neutral technician access handoff guidance before invites and is covered by the local fixture smoke route plan
