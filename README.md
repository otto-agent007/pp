...
- current status: California Compliance RAG V1 plus source-ingestion tooling are implemented as an advisory-only baseline, the dry-run/no-embed source preflight passes locally, the `/auth/update-password` Vercel lambda mapping blocker is fixed, local Windows Vercel packaging is now blocked by a symlink `EPERM`, and no preview/production migration has been applied by Codex; the latest local and preview preflights remain blocked on approved Supabase env names, protected-preview access, and admin/dispatcher sign-in
