...
- current status: California Compliance RAG V1 plus source-ingestion tooling are implemented as an advisory-only baseline, the dry-run/no-embed source preflight passes locally, the compliance advisory route checks schema/RPC readiness before OpenAI embeddings, the customer/admin shared-primitives closeout is locally verified, status/searchability QA is locally verified with fixture Browser route walking, the brand-font direction bundle is design-only guidance, the `/auth/update-password` Vercel lambda mapping blocker is fixed, local Windows Vercel packaging is now blocked by a symlink `EPERM`, and pending migration application remains gated on an approved local or preview target; no preview/production migration has been applied by Codex; the latest Ready preview is `https://pest-patrol-9p9xhuitd-ottoagent007-gmailcoms-projects.vercel.app`; the latest local and preview preflights remain blocked on approved Supabase env names, local Docker/Postgres availability for local Supabase target inspection, protected-preview access, and admin/dispatcher sign-in
