...
| Review compliance workspace | `/compliance` | Source counts, source-readiness workflow cards, Chemical Product Binder summaries/cards, and advisory audit state render; binder status is deterministic from existing records, and when `OPENAI_API_KEY` is absent the page reports RAG disabled without exposing secrets. Run `corepack pnpm compliance:ingest -- --dry-run --no-embed` first for a read-only source preflight, and only rerun without `--dry-run` after an operator-approved local or preview Supabase target and env set are loaded. If `20260516175724_california_compliance_rag_v1.sql` is not applied, the page reports setup required without raw Supabase errors. |
