...
| `/api/csp-report` | POST | unauthenticated by design | none | browser-reported CSP violation details (blocked and document URI, violated directive) | none by design; browsers post violation reports without credentials | `checkApiRateLimit` keyed by client ip; body capped at 8KB and projected onto a fixed bounded field set before `safeLogWarn` sanitizes it; malformed bodies are discarded | no | no, empty 204 | API2, API4, API9, API10 | tested |
...
| `/api/portal/[customerId]/billing` | GET | portal session cookie | `customerId` | customer invoices, line items, payment state | `validatePortalSession` scoped to same `customerId` | `checkApiRateLimit` keyed by client/customer/session; session expiry/revocation | no | yes | API1, API2, API3, API5 | tested |
...
| `/api/portal/[customerId]/sessions` | POST | portal grant exchange | `customerId`, one-time `grant` | portal access token/session hashes, token audit events | same-origin guard; hashed grant lookup scoped to same `customerId`; revoked/expired check | `checkApiRateLimit` keyed by client/customer; one-time grant consumed; HttpOnly/SameSite/Secure session cookie | no | redirects to customer portal | API1, API2, API3, API5, API6 | tested |
...
| `/api/technicians` | GET, POST | admin bearer token | technician profile ids/email input | profiles, invite metadata, technician setup redirect | `getAdminAccess`; service-role invite/list after auth | `checkApiRateLimit` on invite keyed by admin/target email; route validation; provider invite path | Supabase auth invite | no | API1, API2, API3, API5, API6, API10 | tested |
